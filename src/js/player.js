import { downloadTrack, getTrackBlob, getCachedMap, deleteTrack, clearAll, diffTracks } from './download-manager.js'
...
function offlineBtnAttrs(state) {
  switch (state) {
    case 'cached':       return { icon: ICON.trash,    label: 'Remove offline copy' }
    case 'update':       return { icon: ICON.refresh,  label: 'Update available' }
    case 'downloading':  return { icon: ICON.spinner,  label: 'Downloading…' }
    case 'offline-only': return { icon: ICON.trash,    label: 'Remove from device' }
    default:             return { icon: ICON.download, label: 'Download for offline' }
  }
}
...
export class AudioPlayer {
      ...
      try {
        ...
        if (map.size > 0) {
          this.tracks = [...map.values()].map(c => ({
            id: c.id, name: c.name, size: c.size, url: null,
            offlineState: 'offline-only', _cacheKey: c.id,
          }))
        } else {
  ...
  _mergeOfflineState() {
    const { cached, updates, offlineOnly } = diffTracks(this.tracks, this._cachedMap)
    ...
    this.tracks = this.tracks.map(t => {
      if (cachedIds.has(t.id))  return { ...t, offlineState: 'cached', _cacheKey: t.id }
      if (updateMap.has(t.id))  return { ...t, offlineState: 'update', _cacheKey: updateMap.get(t.id) }
      return                           { ...t, offlineState: 'idle',   _cacheKey: null }
    })

    // Append tracks that exist only in cache (removed/renamed on server)
    offlineOnly.forEach(c => this.tracks.push({
      id: c.id, name: c.name, size: c.size, url: null,
      offlineState: 'offline-only', _cacheKey: c.id,
    }))
  }
  ...
  async _downloadTrack(index) {
    ...
    try {
      ...
      this.tracks[index] = { ...track, offlineState: 'cached', _cacheKey: track.id }
      this._cachedMap.set(track.id, { id: track.id, name: track.name, size: track.size })
  ...
  async _downloadAll() {
    const pending = this.tracks
      .map((t, i) => ({ t, i }))
      .filter(({ t }) => t.offlineState === 'idle' || t.offlineState === 'update')
  ...
  _renderTracklist() {
    if (this.tracks.length === 0) {
      if (!this.tracklistEmptyEl.parentNode) this.tracklistEl.appendChild(this.tracklistEmptyEl)
      this.tracklistEmptyEl.textContent = 'No tracks found.'
      return
    }
    if (this.tracklistEmptyEl.parentNode) this.tracklistEmptyEl.remove()
    this.tracklistEl.querySelectorAll('.tracklist__item').forEach(el => el.remove())

    this.tracks.forEach((track, i) => {
      const { icon, label } = offlineBtnAttrs(track.offlineState)
      const item = document.createElement('div')
      item.className = 'tracklist__item'
      if (track.offlineState === 'offline-only') item.classList.add('is-offline-only')
  ...
  _setItemState(index, state) {
    const btn = this.tracklistEl.querySelector(`.tracklist__offline-btn[data-index="${index}"]`)
    if (!btn) return
  ...
  _renderOfflineControls() {
    ...
    const stored  = this.tracks.filter(t => t.offlineState === 'cached' || t.offlineState === 'offline-only')
    const bytes   = stored.reduce((s, t) => s + (t.size || 0), 0)
    const pending = idle + updates

    if (pending === 0 && stored.length === 0) { el.hidden = true; return }
    el.hidden = false

    const btnLabel   = updates > 0 && idle === 0
      ? `Update ${updates} track${updates !== 1 ? 's' : ''}`
      : `Download ${pending} track${pending !== 1 ? 's' : ''}`
    const storageStr = bytes > 0 ? `${(bytes / 1048576).toFixed(1)} MB offline` : ''
    ...
    `
    document.getElementById('btnDownloadAll')?.addEventListener('click', () => this._downloadAll())
    document.getElementById('btnClearOffline')?.addEventListener('click', () => this._clearAllOffline())
  }
  ...
  _togglePlay() {
    if (!this.audio.src) { if (this.tracks.length > 0) this.playTrack(0); return }
    if (this.audio.paused) this.audio.play(); else this.audio.pause()
  }

  _prevTrack() { this.playTrack(this.currentIndex > 0 ? this.currentIndex - 1 : this.tracks.length - 1) }
  _nextTrack() { this.playTrack((this.currentIndex + 1) % this.tracks.length) }

  _onTimeUpdate() {
    const { currentTime, duration } = this.audio
    this.currentTimeEl.textContent = this._formatTime(currentTime)
    if (duration) this.progressFill.style.width = `${(currentTime / duration) * 100}%`
  }
  ...
  _setPlayingState(playing) {
    this.btnPlay.querySelector('.icon-play').style.display  = playing ? 'none' : ''
    this.btnPlay.querySelector('.icon-pause').style.display = playing ? ''     : 'none'
    this.artworkInner?.classList.toggle('is-playing', playing)
  }

  _showLoading(show) { this.loadingEl.hidden = !show }
  ...
  _formatSize(bytes) { return bytes ? `${(bytes / 1048576).toFixed(1)} MB` : '' }
